...
class ExceptionWindow(QWidget):
    def __init__(self, extype, value, tb):
        ...
        information_label.setText(gt('The CDDA Game Launcher just crashed. An '
...
''').format(version=html.escape(version), extype=html.escape(str(extype)),
        ...
        report_url = NEW_ISSUE_URL + '?' + urlencode(
            {'title': gt('Unhandled exception: [Enter a title]'), 'body': gt('''* Description: [Enter what you did and what happened]
* Version: {version}
* Type: `{extype}`
* Value: {value}
* Traceback:
...
''').format(version=version,
        ...
        report_label.setText(gt('Please help us make a better launcher '
        ...
        exit_button.setText(gt('Exit'))
        exit_button.clicked.connect(self.close)
        layout.addWidget(exit_button, 3, 0, Qt.AlignRight)
        self.exit_button = exit_button

        self.setLayout(layout)
        self.setWindowTitle(gt('Something went wrong'))
